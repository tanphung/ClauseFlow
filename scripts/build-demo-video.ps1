...
& ffmpeg -y -v error -i $rawVideo -i $fullAudio `
  -filter_complex "[0:v]trim=start=$trim,setpts=$timelineScale*(PTS-STARTPTS),trim=duration=$sceneDuration,fps=30,scale=1920:1080:flags=lanczos,format=yuv420p[v];[1:a]loudnorm=I=-16:TP=-1.5:LRA=11[a]" `
