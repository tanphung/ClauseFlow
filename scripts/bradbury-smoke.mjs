...
async function completePayment(dealId, canary = false) {
  ...
  if (state.status === "FUNDED" || state.status === "REVISION_REQUIRED") {
    const evidence = canary
      ? [dealId, "https://raw.githubusercontent.com/tanphung/ClauseFlow/f460b6b/contracts/clauseflow.py", "https://raw.githubusercontent.com/tanphung/ClauseFlow/f460b6b/docs/ARCHITECTURE.md", "https://raw.githubusercontent.com/tanphung/ClauseFlow/f460b6b/README.md", "https://github.com/tanphung/ClauseFlow/tree/f460b6b", "Canary delivery: the version-pinned contract source and architecture document expose the independent material validator design. The README and immutable GitHub tree corroborate the reviewed commit; settlement is permitted only if validators substantively confirm every accepted design obligation."]
...
if (mode === "canary") {
  ...
  const canaryTitle = "ClauseFlow validator consensus canary f460b6b";
